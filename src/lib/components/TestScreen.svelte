    ...
    timerInterval = setInterval(() => {
      ...
      Object.keys(tests).forEach((key) => {
        if (tests[key].status === "running" && tests[key].startTime) {
          tests[key].elapsedTime = Math.floor(
            (Date.now() - tests[key].startTime) / 1000,
          );
        }
      });
    }, 500);
  ...
  async function runTests() {
    ...
    if (!backendUrl) {
      updateTest(
        ...
        `Expected: wss://funkhaus-websocket.onrender.com or ws://localhost:3001\nGot: ${backendUrl || "undefined"}`,
      );
      generateErrorReport();
      return;
    }
    ...
    try {
      ...
      if (data.status === "ok" || data.service) {
        updateTest(
          "health",
          "passed",
          `Backend online (${data.service || "Funkhaus"})`,
        );
      } else {
...
<div class="test-screen">
  <div class="test-card">
    ...
    <div class="tests-container">
      ...
      <div class="test-item {getStatusClass(tests.health.status)}">
        ...
        <div class="test-message">
          {getDynamicMessage("health", tests.health)}
        </div>
          ...
          <div class="progress-bar-container">
            <div
              ...
              style="width: {Math.min(
                (tests.health.elapsedTime / 90) * 100,
                100,
              )}%"
            ></div>
      ...
      <div class="test-item {getStatusClass(tests.socket.status)}">
        ...
        <div class="test-message">
          {getDynamicMessage("socket", tests.socket)}
        </div>
        {#if tests.socket.status === "running"}
          <div class="progress-bar-container">
            <div
              class="progress-bar"
              style="width: {Math.min(
                (tests.socket.elapsedTime / 90) * 100,
                100,
              )}%"
            ></div>
    ...
    {#if allTestsPassed}
      <div class="success-message">🎉 All tests passed! Starting app...</div>
    {/if}
